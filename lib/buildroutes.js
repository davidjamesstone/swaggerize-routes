...
var utils = require('./utils'),
  ...
  schemaParameters = require('openapi-jsonschema-parameters')

function getSchemaParameters (parameters) {
  var p = schemaParameters(parameters)
  if (p.body) {
    p.payload = p.body
    delete p.body
  }
  ...
  for (var key in p) {
    if (p[key].required && p[key].required.length === 0) {
...
function buildroutes (options) {
  ...
  Object.keys(api.paths).forEach(function (path) {
    ...
    utils.verbs.forEach(function (verb) {
      ...
      route.ratifyValidators = operation.parameters && getSchemaParameters(operation.parameters)
